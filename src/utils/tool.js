...
const Tool = {
  ...
  getTime(val) {
    ...
    const timeValue = `${year}-${month}-${day} ${hour}:${minute}:${'00'}`
    return timeValue
  },
  debounce: (fn, delay) => {
    let timer = null // 借助闭包
  ...
  // exp: 20200716T195143Z -> 2020-09-19 12:07:26
  parseTime: time => {
    if (!time) return ''
    const arr = time.match(
      new RegExp(
  ...
  getArrType: function (dect) {
    ...
    const res = []
    dect.forEach(e => {
      const type = that.getOneType(e.id)
  ...
  /**
   * 和PHP一样的history1戳格式化函数
   * @param {string} format 格式
   * @param {int} timestamp 要格式化的history1 默认为当前history1
   * @return {string}   格式化的history1字符串
   */
